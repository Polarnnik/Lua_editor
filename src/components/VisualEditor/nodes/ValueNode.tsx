...
export const valueStringDef: NodeDefinition = {
  type: 'value_string',
  label: 'Строка',
  category: 'Значения',
  color: '#db2777',
  ...
  component: ValueNode,
  codegen: {
    evaluate: (node) => literal((node.data as any).value as string || ''),
  },
};

export const valueNumberDef: NodeDefinition = {
  type: 'value_number',
  label: 'Число',
  category: 'Значения',
  color: '#16a34a',
  ...
  component: ValueNode,
  codegen: {
    evaluate: (node) => literal((node.data as any).value as number ?? 0),
  },
};
